...
{
    m_HandlerName=AP4_String_AllocCopy(hdlr_name);
    
    m_Size += 20+AP4_String_length(m_HandlerName)+1;
}
...
AP4_HdlrAtom::WriteFields(AP4_ByteStream& stream)
{
    ...
    if (AP4_FAILED(result)) return result;
    result = stream.Write(AP4_String_c_str(m_HandlerName), AP4_String_length(m_HandlerName)+1);
    if (AP4_FAILED(result)) return result;

    // pad with zeros if necessary
    AP4_Size padding = m_Size-(AP4_FULL_ATOM_HEADER_SIZE+20+AP4_String_length(m_HandlerName)+1);
...
AP4_HdlrAtom::InspectFields(AP4_AtomInspector& inspector)
{
    ...
    inspector.AddField("handler_name", AP4_String_c_str(m_HandlerName));
