...
AP4_UrlAtom::WriteFields(AP4_ByteStream& stream)
{
    if (m_Flags & 1) {
        ...
        AP4_Result result = stream.Write(AP4_String_c_str(m_Url), AP4_String_length(m_Url)+1);
        if (AP4_FAILED(result)) return result;

        // pad with zeros if necessary
        AP4_Size padding = m_Size-(AP4_FULL_ATOM_HEADER_SIZE+AP4_String_length(m_Url)+1);
...
AP4_UrlAtom::InspectFields(AP4_AtomInspector& inspector)
{
    if (m_Flags & 1) {
        ...
        inspector.AddField("location", AP4_String_c_str(m_Url));
    }
