...
AP4_SdpAtom::AP4_SdpAtom(const char* sdp_text) :
    ...
    m_SdpText(sdp_text)*/
{
    m_SdpText=AP4_String_AllocCopy(sdp_text);
    m_Size += AP4_String_length(m_SdpText)+1;
}
...
{
    // sdptext
    AP4_Result result = stream.Write(AP4_String_c_str(m_SdpText), AP4_String_length(m_SdpText));
    if (AP4_FAILED(result)) return result;

    // pad with zeros if necessary
    AP4_Size padding = m_Size-(AP4_ATOM_HEADER_SIZE+AP4_String_length(m_SdpText));
...
{
    inspector.AddField("sdp_text", AP4_String_c_str(m_SdpText));
