...
static void sbr_header(bitfile *ld, sbr_info *sbr)
{
    ...
    conout("bs_amp_res: %d\n", sbr->bs_amp_res);
    conout("bs_start_freq: %d\n", sbr->bs_start_freq);
    conout("bs_stop_freq: %d\n", sbr->bs_stop_freq);
    conout("bs_xover_band: %d\n", sbr->bs_xover_band);
    if (bs_header_extra_1)
    {
        conout("bs_freq_scale: %d\n", sbr->bs_freq_scale);
        conout("bs_alter_scale: %d\n", sbr->bs_alter_scale);
        conout("bs_noise_bands: %d\n", sbr->bs_noise_bands);
    }
    if (bs_header_extra_2)
    {
        conout("bs_limiter_bands: %d\n", sbr->bs_limiter_bands);
        conout("bs_limiter_gains: %d\n", sbr->bs_limiter_gains);
        conout("bs_interpol_freq: %d\n", sbr->bs_interpol_freq);
        conout("bs_smoothing_mode: %d\n", sbr->bs_smoothing_mode);
    }
    conout("\n");
#endif
}
