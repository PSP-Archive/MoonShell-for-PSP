...
#if 0
    conout("f_master[%d]: ", nrBands);
    for (k = 0; k <= nrBands; k++)
    {
        conout("%d ", sbr->f_master[k]);
    }
    conout("\n");
#endif
...
#if 0
    conout("f_master[%d]: ", sbr->N_master);
...
#if 0
    conout("bs_freq_scale: %d\n", sbr->bs_freq_scale);
    conout("bs_limiter_bands: %d\n", sbr->bs_limiter_bands);
    conout("f_table_res[HI_RES][%d]: ", sbr->N_high);
    for (k = 0; k <= sbr->N_high; k++)
    {
        conout("%d ", sbr->f_table_res[HI_RES][k]);
    }
    conout("\n");
#endif
#if 0
    conout("f_table_res[LO_RES][%d]: ", sbr->N_low);
    for (k = 0; k <= sbr->N_low; k++)
    {
        conout("%d ", sbr->f_table_res[LO_RES][k]);
    }
    conout("\n");
#endif
...
#if 0
    conout("f_table_noise[%d]: ", sbr->N_Q);
    for (k = 0; k <= sbr->N_Q; k++)
    {
        conout("%d ", sbr->f_table_noise[k] - sbr->kx);
    }
    conout("\n");
#endif
...
#if 0
    conout("f_table_lim[%d][%d]: ", 0, sbr->N_L[0]);
    for (k = 0; k <= sbr->N_L[0]; k++)
    {
        conout("%d ", sbr->f_table_lim[0][k]);
    }
    conout("\n");
#endif
...
#if 0
        conout("f_table_lim[%d][%d]: ", s, sbr->N_L[s]);
        for (k = 0; k <= sbr->N_L[s]; k++)
        {
            conout("%d ", sbr->f_table_lim[s][k]);
        }
        conout("\n");
#endif
    }
