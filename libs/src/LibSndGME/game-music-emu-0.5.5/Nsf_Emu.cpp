...
struct Nsf_File : Gme_Info_
{
	...
	blargg_err_t load_( Data_Reader& in )
	{
		...
		if ( h.chip_flags & ~(fds_flag | namco_flag | vrc6_flag | fme7_flag) )
...
static byte fds_initial_banks[2];
...
blargg_err_t Nsf_Emu::load_( Data_Reader& in )
{
	...
	if ( load_addr < sram_addr || init_addr < sram_addr )
	{
	...
	for ( int i = 0; i < bank_count; i++ ){
	  if ( header_.banks [i] ) UseBankSwitch=true;
	}
	if(UseBankSwitch==true){
	  // bank-switched
	  for(int idx=0;idx<bank_count;idx++){
	    initial_banks[idx]=header_.banks[idx];
	  }
	  fds_initial_banks[0]=initial_banks[6+0];
	  fds_initial_banks[1]=initial_banks[6+1];
	  }else{
	  printf("Make default bank map. load_addr=$%04x\n",load_addr);
	  for ( int i = -2; i < bank_count; i++ ){
	    int first_bank = load_addr / bank_size;
	    int bank = 8-first_bank+i;
	    if(bank<0) bank=0;
	    if ( bank >= (unsigned) total_banks ) bank = 0;
	    if(i<0){
	      fds_initial_banks[i+2]=bank;
	      }else{
	...
	printf("FDS initial banks: ");
	for(int idx=0;idx<2;idx++){
	  printf("%d ",fds_initial_banks [idx]);
	}
	printf("\n");
	printf("ROM initial banks: ");
	for(int idx=0;idx<bank_count;idx++){
	  printf("%d ",initial_banks [idx]);
	}
	printf("\n");
...
extern bool GME_NSF_UseFDSChip;
bool GME_NSF_UseFDSChip;
...
blargg_err_t Nsf_Emu::start_track_( int track )
{
	...
	if ( header_.chip_flags & fds_flag ) {
	  GME_NSF_UseFDSChip=true;
	  for(int page=0;page<2;page++){
	    int bank=fds_initial_banks[page];
	    printf("FDS page:%d bank:%d\n",page,bank);
	    blargg_long offset = rom.mask_addr( bank * (blargg_long) bank_size );
	    if ( offset >= rom.size() ) set_warning( "Invalid bank" );
	    byte *prom=rom.at_addr( offset );
	    for(int idx=0;idx<bank_size;idx++){
	      sram[(page*bank_size)+idx]=prom[idx];
	    }
	  }
	  }else{
	  GME_NSF_UseFDSChip=false;
