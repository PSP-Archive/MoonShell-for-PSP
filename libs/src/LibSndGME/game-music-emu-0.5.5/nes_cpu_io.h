...
#include "virtuanessrc097/APU_FDS.h"
...
int Nsf_Emu::cpu_read( nes_addr_t addr )
{
	...
	if((0x4040<=addr)&&(addr<0x40c0)){ // FDS Wavetable
//	  printf("FDS WT: Read $%4x.\n",addr);
	  return(pVNES_APU_FDS->Read(addr));
	}
...
void Nsf_Emu::cpu_write( nes_addr_t addr, int data )
{
	...
	if((0x4040<=addr)&&(addr<0x40c0)){ // FDS Wavetable
//	  printf("FDS WT: Write $%4x,$%2x.\n",addr,data);
	  pVNES_APU_FDS->Write(addr,data);
	  return;
	}
	...
	if((addr==0x5FF6)||(addr==0x5FF7)){
	  int page=addr-0x5FF6;
	  int bank=data;
	  printf("FDS bank switch: page:%d bank:%d\n",page,bank);
	...
	if(GME_NSF_UseFDSChip==true){
	  if(0x8000<=addr){
	    BOOST::uint8_t *p=(BOOST::uint8_t*)cpu::get_code( addr );
	    if(p!=NULL) *p=data;
	  }
	}
