...
//      conout("ccModulation(%x):%d\n",trk,_Track->ccModulation);
    ...
    case 64: {
      if(MIDIEmu_Settings_ShowEventMessage==true) conout("ccSustainPedal(%x):%d\n",trk,data1);
      if(data1<64){
        _Track->ccSustainPedal=false;
        PCH_PedalOff(trk);
        }else{
    ...
    case 91: {
      if(MIDIEmu_Settings_ShowEventMessage==true) conout("ccReverb(%x):%d\n",trk,data1);
...
//  conout("Map%d,%d,",trk,_Track->DrumMap);
//  conout("NoteOn:Trk.%d, Note.%d Prg.%d\n",trk,Note,_Track->Program);
...
void MTRK_SetExMap(u32 trk,u32 mode)
{
  ...
  conout("sMap%d:%d,",trk,_Track->DrumMap);
  
  for(u32 idx=0;idx<MTRK_TrackCount;idx++){
    conout("%d",_MTRK->Track[idx].DrumMap);
  }
  conout("\n");
}
