...
static void RCP_Init(void)
{
  ...
  pRCP_Chank=(TRCP_Chank*)malloc(sizeof(TRCP_Chank));
...
static void RCP_Free(void)
{
  if(pRCP!=NULL){
    for(u32 trk=0;trk<RCPTrackMax;trk++){
      if(pRCP->RCP_Track[trk].pLoop!=NULL){
        free(pRCP->RCP_Track[trk].pLoop); pRCP->RCP_Track[trk].pLoop=NULL;
      }
      if(pRCP->RCP_Track[trk].pExc!=NULL){
        free(pRCP->RCP_Track[trk].pExc); pRCP->RCP_Track[trk].pExc=NULL;
      }
    }
    free(pRCP); pRCP=NULL;
  }
  ...
  if(pRCP_Chank!=NULL){
    free(pRCP_Chank); pRCP_Chank=NULL;
  }
}
...
static void RCP_LoadTrackChank(TRCP_Track *pRCP_Track)
{
  ...
  pRCP_Track->PlayMode=(TRCP_Track::EPlayMode)RCP_ReadByte();
...
*/
  ...
  pRCP_Track->pLoop=(TRCP_Track_Loop*)malloc(sizeof(TRCP_Track_Loop)*RCP_Track_LoopMax);
  ...
  pRCP_Track->pExc=(TRCP_Track_Exc*)malloc(sizeof(TRCP_Track_Exc));
  
  TRCP_Track_Exc *pExc=pRCP_Track->pExc;
  
  pExc->cmd=TRCP_Track_Exc::ECmd_None;
  pExc->GT=0;
  pExc->Vel=0;
  pExc->Count=0;
...
static void RCP_Proc_LoopStart(TRCP_Track *pRCP_Track)
{
  if((pRCP_Track->LoopFreeIndex+1)==RCP_Track_LoopMax){
    conout("Loop nest buffer overflow.\n");
    return;
  }
...
static void RCP_Proc_LoopEnd(TRCP_Track *pRCP_Track,u32 LoopCount)
{
  ...
  if(pLoop->LoopCount==0xffffffff){
    if(LoopCount==0){
      conout("Infinite loop was not supported.\n");
      LoopCount=2;
    }
...
static void ExcStart(TRCP_Track_Exc *pExc,TRCP_Track_Exc::ECmd _ExcCmd,u32 GateTime,u32 Vel)
{
  pExc->cmd=_ExcCmd;
  
  if(pExc->cmd!=TRCP_Track_Exc::ECmd_CHExclusive){
    pExc->cmd=TRCP_Track_Exc::ECmd_None;
    return;
  }
...
{
  if(pExc->cmd==TRCP_Track_Exc::ECmd_None) return;
  ...
  if((data[0]!=0x41)||(data[1]!=0x10)||(data[2]!=0x42)||(data[3]!=0x12)){
    if(ShowMessage==true) conout("not support pExc-> format.\n");
    return;
  }
  
  if(data[4]!=0x40){
    if(ShowMessage==true) conout("not support !send pExc-> format.\n");
    return;
  }
...
static void UserExcSend(bool ShowMessage,u32 ch,TRCP_Track_Exc *pExc,u32 ExcID,u32 GateTime,u32 Vel)
{
  ...
  ExcStart(pExc,TRCP_Track_Exc::ECmd_CHExclusive,GateTime,Vel);
...
static void RCP_ProcRCP(bool ShowMessage,bool EnableNote,TRCP_Track *pRCP_Track)
{
  ...
  if(pRCP_Track->PlayMode==TRCP_Track::EPM_Mute){
    pRCP_Track->EndFlag=true;
    return;
  }
...
//    conout("%d,%02x:%02x:%02x:%02x\n",ch,cmd,step,data0,data1);
    if((0x90<=cmd)&&(cmd<=0x97)){
      ...
      TRCP_Track_Exc::ECmd ExcCmd=(TRCP_Track_Exc::ECmd)cmd;
      if((ExcCmd==TRCP_Track_Exc::ECmd_CHExclusive)||(ExcCmd==TRCP_Track_Exc::ECmd_Exec)||(ExcCmd==TRCP_Track_Exc::ECmd_Commment)){
        ExcStart(pRCP_Track->pExc,ExcCmd,data0,data1);
        if(ExcCmd!=TRCP_Track_Exc::ECmd_CHExclusive) step=0;
        }else{
...
//              if(ShowMessage==true) conout("Pitch Bend(%x):%d\n",ch,PitchBend);
            ...
            case 0xf5: { // Key Change       | F5          | Key Data     |      -       |      -       
              if(ShowMessage==true) conout("Key Change(%x):%d,%d,%d\n",ch,step,data0,data1);
              step=0;
            } break;
