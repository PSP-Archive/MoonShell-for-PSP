...
{
  if(gME_Text!=NULL){
    free(gME_Text); gME_Text=NULL;
  }
  if(gME_Copyright!=NULL){
    free(gME_Copyright); gME_Copyright=NULL;
  }
  if(gME_Title!=NULL){
    free(gME_Title); gME_Title=NULL;
  }
}
...
#define ReadData2() { \
  ...
  switch(cmd){
    case 0x8:
      ReadData2();
      if(ShowMessage==true) conout("$%xx/NoteOff(%x):note%d vel%d\n",cmd,ch,Data0,Data1);
      if(EnableNote==true) MTRK_NoteOff(ch,Data0,Data1);
      break;
    case 0x9:
      // if(ch==1) EnableNote=false;
      ReadData2();
      if(ShowMessage==true) conout("$%xx/NoteOn(%x):note%d vel%d\n",cmd,ch,Data0,Data1);
      if(Data1!=0){ // Vel!=0
        if(EnableNote==false){
    ...
    case 0xa:
      ReadData2();
//      if(ShowMessage==true) conout("$%xx/PolyKeyPress(%x):note%d vel%d\n",cmd,ch,Data0,Data1);
      break;
    case 0xb:
      ReadData2();
      if(ShowMessage==true) conout("$%xx/ControlChange(%x):Num%d Val%d\n",cmd,ch,Data0,Data1);
      MTRKCC_Proc(ch,Data0,Data1);
      break;
    case 0xc:
      ReadData1();
      if(ShowMessage==true) conout("$%xx/ProgramChange(%x):prg%d\n",cmd,ch,Data0);
      MTRK_SetProgram(ch,Data0);
      break;
    case 0xd:
      ReadData1();
//      if(ShowMessage==true) conout("$%xx/ChannelPress(%x):vel%d\n",cmd,ch,Data0);
      break;
    case 0xe:
      ReadData2();
      s32 PitchBend=(((s32)Data1) << 7)+((s32)Data0);
      PitchBend-=8192;
//      if(ShowMessage==true) conout("$%xx/PitchBend(%x):%d\n",cmd,ch,PitchBend);
...
{
  if(ShowMessage==true) conout("!SysExEventF0\n");
  ...
  if((data[0]!=0x41)||(data[1]!=0x10)||(data[2]!=0x42)||(data[3]!=0x12)){
    if(ShowMessage==true) conout("not support exc format.\n");
    return;
  }
  
  if(data[4]!=0x40){
    if(ShowMessage==true) conout("not support !send exc format.\n");
    return;
  }
  ...
  if(((data[5]&0xf0)==0x10)&&(data[6]==0x15)){
    ...
    switch(mode){
      case 0: {
        conout("Ch%d:Use For Rhythm Part to OFF\n",ch);
        MTRK_SetExMap(ch,mode);
      } break;
      case 1: {
        conout("Ch%d:Use For Rhythm Part to Map1\n",ch);
        MTRK_SetExMap(ch,mode);
      } break;
      case 2: {
        conout("Ch%d:Use For Rhythm Part to Map2\n",ch);
        MTRK_SetExMap(ch,mode);
      } break;
      default: {
        conout("Ch%d:Use For Rhythm Part to Unknown(%d)\n",ch,mode);
      } break;
    }
...
{
  if(ShowMessage==true) conout("!SysExEventF7\n");
...
void SM_ProcMetaEvent(TSM_Track *pSM_Track)
{
  ...
  SM_Event.MetaEvent.Data=(u8*)malloc(len+1);
  ...
  const bool ShowMessage=MIDIEmu_Settings_ShowEventMessage;
  
  switch(SM_Event.MetaEvent.ID){
    case 0x00:
      if(ShowMessage==true) conout("Meta:SeqNo\n");
      break;
    case 0x01:
      if(ShowMessage==true) conout("Meta:Text:%s\n",SM_Event.MetaEvent.Data);
      if(gME_Text==NULL){
        gME_Text=(char*)malloc(strlen((char*)SM_Event.MetaEvent.Data)+1);
        strcpy(gME_Text,(char*)SM_Event.MetaEvent.Data);
      }
      break;
    case 0x02:
      if(ShowMessage==true) conout("Meta:Copyright:%s\n",SM_Event.MetaEvent.Data);
      if(gME_Copyright==NULL){
        gME_Copyright=(char*)malloc(strlen((char*)SM_Event.MetaEvent.Data)+1);
        strcpy(gME_Copyright,(char*)SM_Event.MetaEvent.Data);
      }
      break;
    case 0x03:
      if(ShowMessage==true) conout("Meta:Title:%s\n",SM_Event.MetaEvent.Data);
      if(gME_Title==NULL){
        gME_Title=(char*)malloc(strlen((char*)SM_Event.MetaEvent.Data)+1);
        strcpy(gME_Title,(char*)SM_Event.MetaEvent.Data);
      }
      break;
    case 0x04:
      if(ShowMessage==true) conout("Meta:InstName:%s\n",SM_Event.MetaEvent.Data);
      break;
    case 0x05:
      if(ShowMessage==true) conout("Meta:Liq:%s\n",SM_Event.MetaEvent.Data);
      break;
    case 0x06:
      if(ShowMessage==true) conout("Meta:Marker:%s\n",SM_Event.MetaEvent.Data);
      break;
    case 0x07:
      if(ShowMessage==true) conout("Meta:QPoint:%s\n",SM_Event.MetaEvent.Data);
      break;
    case 0x08:
      if(ShowMessage==true) conout("Meta:ProgramName:%s\n",SM_Event.MetaEvent.Data);
      break;
    case 0x09:
      if(ShowMessage==true) conout("Meta:DeviceName:%s\n",SM_Event.MetaEvent.Data);
      break;
    case 0x20:
      if(ShowMessage==true) conout("Meta:ChannelPrefix:%x\n",SM_Event.MetaEvent.Data[0]);
      break;
    case 0x21:
      if(ShowMessage==true) conout("Meta:SetPort:%x\n",SM_Event.MetaEvent.Data[0]);
      break;
    case 0x2f:
      conout("Meta:EndTrack\n");
    ...
    case 0x51:
      {
        ...
        if(ShowMessage==true) conout("Meta:Tempo %dusec/%dclk\n",Tempo,_StdMIDI->SM_Chank.TimeRes);
        SM_ProcMetaEvent_SetTempo(Tempo);
      }
      break;
    case 0x54:
      if(ShowMessage==true) conout("Meta:SMPTE\n");
      break;
    case 0x58:
      if(ShowMessage==true) conout("Meta:beatup\n");
      break;
    case 0x59:
      if(ShowMessage==true) conout("Meta:beatdown\n");
      break;
    case 0x7f:
      if(ShowMessage==true) conout("Meta:SeqMetaEvent\n");
      break;
    default:
      if(ShowMessage==true) conout("Meta:UnknownMetaData\n");
      break;
  }
  
  free(SM_Event.MetaEvent.Data); SM_Event.MetaEvent.Data=NULL;
}
...
void SM_LoadStdMIDI(u8 *FilePtr,u32 SampleRate)
{
  ...
  if(MIDIEmu_Settings_ShowInfomationMessages==true){
    TSM_Chank *_SM_Chank=&StdMIDI.SM_Chank;
    conout("Chank:Len=%d frm=%d trk=%d res=%d\n",_SM_Chank->Len,_SM_Chank->Format,_SM_Chank->Track,_SM_Chank->TimeRes);
  }
