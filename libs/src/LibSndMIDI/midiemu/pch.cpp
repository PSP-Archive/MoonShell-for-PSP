...
#include "pch_powertbl_bin.h"
static const u16 *ppowertbl=powertbl_bin;
#define POWRANGE (1024)

#include "pch_sintbl_bin.h"
...
static TPCMStack pPCMStack[PCMStackCount];
...
static TChannelInfo pChannelInfo[ChannelInfoCount];
...
static u32 PCHCount;
#define PCHCount_Max (64)
TPCH pPCH[PCHCount_Max];
...
static bool safemalloc_CallBack_RequestFreeMemory_PlugMIDI_PCH(void)
{
  ...
  if(MaxClock==0){
//    conout("Not found unused program patch. retry all program patch.\n");
    for(u32 var=0;var<128;var++){
      TVariationMap *pVarMap;
      pVarMap=pVarMap_Tone;
      if(pVarMap!=NULL){
    ...
    if(MaxClock==0){
      conout("Not found program patch.\n");
      return(false);
    }
  }
  
//  conout("Unload program patch. Unused clock: %dclks. [%d:%d]\n",MaxClock,tagvar,tagprg);
...
{
  pVarMap_Tone=(TVariationMap*)malloc(sizeof(TVariationMap));
  pVarMap_Drum=(TVariationMap*)malloc(sizeof(TVariationMap));
  ...
  for(int prg=0;prg<128;prg++){
    if(ToneOffset[prg]!=0){
      pVarMap_Tone->pPrgMaps[prg]=(TProgramMap*)malloc(sizeof(TProgramMap));
  ...
  for(int prg=0;prg<128;prg++){
    if(DrumOffset[prg]!=0){
      pVarMap_Drum->pPrgMaps[prg]=(TProgramMap*)malloc(sizeof(TProgramMap));
...
static void PCH_FreeProgramMap_Free_Patch(TProgramPatch *pPrgPatch)
{
  ...
  free(pPrgPatch->pPrg); pPrgPatch->pPrg=NULL;
}
...
static void PCH_FreeProgramMap_Free(TProgramMap *pPrgMap)
{
  for(u32 idx=0;idx<128;idx++){
    if(pPrgMap->ppData[idx]!=NULL){
      ...
      free(pPrgMap->ppData[idx]); pPrgMap->ppData[idx]=NULL;
    }
  }
}
  ...
  if(pVarMap_Tone!=NULL){
    for(u32 idx=0;idx<128;idx++){
      if(pVarMap_Tone->pPrgMaps[idx]!=NULL){
        ...
        free(pVarMap_Tone->pPrgMaps[idx]); pVarMap_Tone->pPrgMaps[idx]=NULL;
      }
    }
    free(pVarMap_Tone); pVarMap_Tone=NULL;
  }
  ...
  if(pVarMap_Drum!=NULL){
    for(u32 idx=0;idx<128;idx++){
      if(pVarMap_Drum->pPrgMaps[idx]!=NULL){
        ...
        free(pVarMap_Drum->pPrgMaps[idx]); pVarMap_Drum->pPrgMaps[idx]=NULL;
      }
    }
    free(pVarMap_Drum); pVarMap_Drum=NULL;
  }
  
  {
    ...
    for(u32 idx=0;idx<PCMStackCount;idx++){
      ...
      if((_pPCMStack->pData!=NULL)||(_pPCMStack->RefCount!=0)){
        conout("PCMStack.%d: Ref=%d, ofs=%d, pData=0x%08x.\n",_pPCMStack->RefCount,_pPCMStack->PCMFileOffset,_pPCMStack->pData);
        err=true;
      }
    }
    if(err==true) StopFatalError(15510,"PCMStack: Not free pointer or zero referrer count founded.\n");
  }
}
...
static void PCMStack_Regist(u32 ofs,void *p)
{
  ...
  StopFatalError(15511,"PCMStack_Regist: Stack buffer overflow.\n");
}
  ...
  for(u32 idx=0;idx<PCMStackCount;idx++){
    ...
    if(_pPCMStack->RefCount!=0){
      if(_pPCMStack->pData==p){
        _pPCMStack->RefCount--;
        if(_pPCMStack->RefCount==0){
          free(_pPCMStack->pData); _pPCMStack->pData=NULL;
        }
        return;
      }
    }
...
//    conout("%d,%d,%d,%d,%d\n",patidx,NoteFreq>>16,NearFreq>>16,RootFreq>>16,NearPatchIndex);
  ...
  if(NearPatchIndex==0x100){
    conout("find near root freq from patch list??\n");
    return(NULL);
  }
...
/*
  if(DrumMode==true){
    conout("LoadDrum:%d,%d->%d,%d\n",prg,Note,VarNum,PrgNum);
    }else{
    conout("LoadTone:%d,%d->%d,%d\n",var,prg,VarNum,PrgNum);
  }
*/
  
  pPrgMap->ppData[PrgNum]=(TProgramPatch*)malloc(sizeof(TProgramPatch));
  if(pPrgMap->ppData[PrgNum]==NULL){
    MemoryOverflowFlag=true;
    conout("malloc(%d); out of memory!!\n",sizeof(TProgramPatch));
    return(false);
  }
  ...
  pPrgPatch->pPrg=(TProgram*)malloc(sizeof(TProgram)*patcount);
  if(pPrgPatch->pPrg==NULL){
    MemoryOverflowFlag=true;
    conout("PatchLoad malloc(%d); out of memory!!\n",sizeof(TProgram)*patcount);
    pPrgPatch->PatchCount=0;
  ...
  for(u32 patidx=0;patidx<patcount;patidx++){
    ...
    if(false){
      conout("EnvRate=");
      for(int idx=0;idx<6;idx++){
        conout("%02x,",pPrg->EnvRate[idx]);
      }
      conout("\n");
      conout("EnvOfs =");
      for(int idx=0;idx<6;idx++){
        conout("%02x,",pPrg->EnvOfs[idx]);
      }
      conout("\n");
    }
...
#define TTACID (0x43415454)
    ...
    switch(ID){
      case WAVEID: {
        ...
        pPrg->pData=malloc(datasize);
        if(pPrg->pData==NULL){
          memoryoverflow=true;
          break;
        }
        
//        conout("Read RAWPCM. %dbytes\n",datasize);
  ...
  if(memoryoverflow==true){
    MemoryOverflowFlag=true;
    conout("PatchLoad out of memory!!\n");
    if(pPrgPatch->pPrg!=NULL){
      for(u32 patidx=0;patidx<pPrgPatch->PatchCount;patidx++){
        TProgram *pPrg=&pPrgPatch->pPrg[patidx];
...
bool PCH_Init(u32 _SampleRate,u32 _GenVolume,u32 _PCHCount)
{
  ...
  {
    PCHCount=_PCHCount;
    if(PCHCount_Max<PCHCount){
      conout("PCH_Init: %d is overflow. set to %dchs.\n",PCHCount,PCHCount_Max);
      PCHCount=PCHCount_Max;
    }
    for(u32 ch=0;ch<PCHCount;ch++){
      PCH_ChInit(ch);
  ...
  for(int idx=0;idx<128;idx++){
    ...
    Note2FreqTableFix16[idx]=(u32)(basefreq*(0x10000+(u32)ppowertbl[(Note*POWRANGE)]));
  }
...
void PCH_NextClock(void)
{
    ...
    if(_pPCH->Enabled==true){
      ...
      if(_pPCH->pChannelInfo->DrumMode==true){
        if((120*1.5)==_pPCH->OnClock){
          if(_pPCH->EnvState!=TPCH::EES_Release5){
            _pPCH->EnvRelLevel=_pPCH->EnvCurLevel;
            _pPCH->EnvState=TPCH::EES_Release5;
  ...
  for(u32 ch=0;ch<PCHCount;ch++){
    ...
    if(_pPCH->VibEnable==true){
      ...
      _pPCH->VibPhase=sintbl_bin[_pPCH->VibCur>>16]*_pPCH->VibDepth/0x10000;
  ...
  for(u32 ch=0;ch<PCHCount;ch++){
    ...
    if(_pPCH->ModEnable==true){
      ...
      _pPCH->ModPhase=sintbl_bin[_pPCH->ModCur>>16]*_pPCH->ModDepth/0x10000;
    }
  }
...
//        conout("d%x\n",_pPCH->EnvCurLevel);
        ...
        if(next==true){
          if(true){ // if(_pPCH->DrumMode==false){
            _pPCH->EnvState=TPCH::EES_Sustain;
            _pPCH->EnvCurLevel=_pPCH->EnvEndLevel;
            ...
            _pPCH->EnvRelLevel=_pPCH->EnvCurLevel;
            _pPCH->EnvState=TPCH::EES_Release3;
...
//        conout("d%x\n",_pPCH->EnvCurLevel);
        ...
        if(next==true){
          _pPCH->EnvState=TPCH::EES_Release4;
          _pPCH->EnvCurLevel=_pPCH->EnvEndLevel;
...
//        conout("r%x\n",_pPCH->EnvCurLevel);
    ...
    if(TPCH::EES_Release3<=_pPCH->EnvState){
...
void PCH_ChangeVolume(u32 trk,u32 v)
{
  if(ChannelInfoCount<=trk){
    conout("Channel overflow. ch=%d",trk);
    return;
  }
...
//  TChannelInfo *_pCI=&pChannelInfo[trk];
  ...
  for(u32 ch=0;ch<PCHCount;ch++){
    ...
    if((_pPCH->Enabled==true)&&(_pPCH->ChannelNum==trk)){
      ...
      _pPCH->FreqAddFix16=FreqTransAddFix16(basefreq,(u32)ppowertbl[(Note*POWRANGE)+TunePitch],pPrg->SampleRate,pPrg->RootFreq);
    }
  }
}
...
static int PCH_FindDisableChannel(u32 PlayChOnly)
{
  ...
  conout("PCH_FindDisableChannel: Can not found diabled channel.");
  return(-1);
}
...
void PCH_NoteOn(u32 trk,u32 GT,s32 Note,s32 Pitch,u32 Vol,u32 Exp,u32 Vel,u32 var,u32 prg,u32 panpot,u32 reverb,bool DrumMode,u32 ModLevel)
{
  ...
  if(ch==-1){
    conout("PCHON %d,%d Error ChannelEmpty.\n",trk,Note);
    return;
  }
  ...
  if(pPrg->EnvelopeFlag==false){
    ...
    _pPCH->EnvState=TPCH::EES_Attack;
  ...
  if(_pPCH->Note==0){
    ...
    _pPCH->FreqAddFix16=FreqTransAddFix16(basefreq,(u32)ppowertbl[(Note*POWRANGE)+Pitch],pPrg->SampleRate,pPrg->RootFreq);
    _pPCH->FreqCurFix16=0;
      ...
      if((_pPCH->ChannelNum==trk)&&(_pPCH->Note==Note)){
        ...
        if(_pCI->Pedal==false){
          if((_pPCH->pPrg->EnvelopeFlag==false)||(_pPCH->EnvCurLevel==0)){
            ...
            if(_pPCH->EnvState<TPCH::EES_Release3){
              _pPCH->EnvRelLevel=_pPCH->EnvCurLevel;
              _pPCH->EnvState=TPCH::EES_Release3;
              _pPCH->EnvSpeed=-_pPCH->pPrg->EnvRate[3];
              _pPCH->EnvEndLevel=(_pPCH->pPrg->EnvOfs[3]*_pPCH->EnvRelLevel)>>16;
...
static __attribute__ ((noinline)) void PCH_Render_Loop(int ch,u32 FreqAddFix16,s32 *pbuf,int SampleCount)
{
  ...
  s32 FreqCurFix16=_pPCH->FreqCurFix16;
  ...
  if(pPrg->s16Flag==false){
    ...
    if(FreqAddFix16<=0x10000){
      for(u32 idx=0;idx<SampleCount;idx++){
        s32 cursample=((LastSampleData*(0x10000-FreqCurFix16))+(CurSampleData*FreqCurFix16))>>16;
        pbuf[0]+=(cursample*lvol)>>16;
        pbuf[1]+=(cursample*rvol)>>16;
        pbuf+=2;
        
        FreqCurFix16+=FreqAddFix16;
        if(0x10000<=FreqCurFix16){
          // Get next sample one.
      ...
      for(u32 idx=0;idx<SampleCount;idx++){
        ...
        if(PrgPos<EndPoint){
          LastSampleData=pData[PrgPos-1]<<8;
          CurSampleData=pData[PrgPos]<<8;
          }else{
          if(LoopStart==0x10000000){
            pbuf=NULL;
          ...
          PrgPos=PrgPos-EndPoint+LoopStart;
...
void PCH_RenderStart(u32 SampleCount)
{
  ...
  for(u32 ch=0;ch<PCHCount;ch++){
    ...
    if((_pPCH->pPrg!=NULL)&&(_pPCH->FreqAddFix16!=0)){
      totalch++;
      if(0x10000<_pPCH->FreqAddFix16) heavych+=3;
    }
  }
  heavych/=2;
...
/*
  ...
  if(a<totalch){
    a=totalch;
    conout("------ Total channels: %dchs.\n",totalch);
  }
*/
  
  if((PCHCount-1)<heavych) heavych=PCHCount-1;
