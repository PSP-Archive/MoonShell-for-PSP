...
//    conout("BitStream: Remain=0x%xbytes, size_in_bits=%d\n",s->gb.buffer_end-s->gb.buffer,s->gb.size_in_bits);
//    conout("FilePos=%d\n",csi->curpos);
   ...
   /* compute current block length */
    if (s->use_variable_block_len)
    {
        ...
        if (s->reset_block_lengths)
        {
            ...
            {
                conout("prev_block_len_bits %d out of range\n", s->frame_len_bits - v);
                return -2;
            }
            ...
            if (v >= s->nb_block_sizes)
            {
                conout("block_len_bits %d out of range\n", s->frame_len_bits - v);
                return -3;
            }
        ...
        if (v >= s->nb_block_sizes)
        {
            conout("next_block_len_bits %d out of range\n", s->frame_len_bits - v);
            return -4;        //this is it
        }
   ...
   /* finally compute the MDCT coefficients */
    ...
    { 
        ...
        static int32_t scratch_buf[BLOCK_MAX_SIZE * MAX_CHANNELS] IBSS_ATTR MEM_ALIGN_ATTR;
        if (s->channel_coded[ch])
        {
            int n4, index;
