 ...
 *                     \/            \/     \/    \/            \/
 * $Id: asf.c 29805 2011-05-01 11:42:41Z Buschel $
 *
...
#define ASF_MAX_REQUEST (1L<<15) /* 32KB */
    ...
    csi->advance_buffer(wfx->packet_size-bytesread-length);
    datap = buf;
...
//        conout("media_object_number=%d\n",GETVALUE2b((packet_property >> 4) & 0x03, datap));
        datap += GETLEN2b((packet_property >> 4) & 0x03);
//        conout("media_object_offset=%d\n",GETVALUE2b((packet_property >> 2) & 0x03, datap));
        datap += GETLEN2b((packet_property >> 2) & 0x03);
        replicated_length = GETVALUE2b(packet_property & 0x03, datap);
//        conout("replicated_length=%d\n",replicated_length);
...
//    conout("loaded size=%d, padding_length=%d.\n",datap-buf);
    
    u32 loadedsize=datap-buf;
//    if((loadedsize+padding_length)!=length) return ASF_ERROR_INVALID_LENGTH;
...
int asf_seek(int ms, asf_waveformatex_t* wfx)
{
    ...
    int last_packet = csi->id3->filesize / wfx->packet_size;
    ...
    while (1)
    {
        ...
        if (time+duration>=ms && time<=ms) {
            ...
            DEBUGF("Seek again. ms=%d, time:%d, duration=%d.\n", ms,time,duration);
            /*seek again*/
            delta = ms-time;
...
int asf_skip_frame(asf_waveformatex_t* wfx,int *duration,int *send_time)
{
    uint8_t tmp8, packet_flags, packet_property;
    int ec_length, opaque_data, ec_length_type;
    int datalen;
    ...
    *duration = get_short_le(datap);
    ...
    csi->advance_buffer(wfx->packet_size-bytesread);
