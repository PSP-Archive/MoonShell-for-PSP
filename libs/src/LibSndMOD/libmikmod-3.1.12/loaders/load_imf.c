...
#ifdef SUNOS
extern int fconout(FILE *, const char *, ...);
#endif
	...
	if((eff)&&(eff!=255))
		switch (eff) {
			...
			case 0x11:	/* not slide down */
				fconout(stderr,"\rIMF effect 0x10/0x11 (note slide)"
			...
			case 0x19:	/* set fine sample offset */
				fconout(stderr,"\rIMF effect 0x19 (fine sample offset)"
...
#undef IMF_ProcessEnvelope
		...
		if(ih.pitflg&1) {
			d->pitflg&=~EF_ON;
#ifdef MIKMOD_DEBUG
			fconout(stderr, "\rFilter envelopes not supported yet\n");
#endif
		}
