...
static void store_pdx_data( PDX_DATA *pdx, const u8 *buf, const u32 buflen );
static void adpcm2pcm( int *, const u8 *, long );

/* ------------------------------------------------------------------ */

PDX_DATA *mdx_open_pdx( const u8 *ppdxbuf,const u32 pdxbufsize )
{
  PDX_DATA* pdx = NULL;
  ...
  store_pdx_data( pdx, ppdxbuf, pdxbufsize );
  ...
  while(1) {
    ...
    for ( i=0 ; i<MDX_MAX_PDX_TONE_NUMBER ; i++,num++ ) {
      ...
      if (!pdx->tone[num].orig_data) {
        conout("PDX.%d: 16bitsPCM Memory overflow. %dbytes.\n",num,sizeof(int)*size/2);
        goto error_end;
      }
      ...
      for ( j=0 ; j<pdx->tone[num].orig_size ; j++ ) {
        if (address+j*2+1<buflen) {
          ...
          conout("PDX.%d: 16bitsPCM Address overflow. %d>=%d.\n",num,address+j*2+1,buflen);
          goto error_end;
        }
      ...
      if (size>=2*1024*1024) {
        ...
        if (!pdx->tone[num].data) {
          conout("PDX.%d: RAW16 Memory overflow. %dbytes.\n",num,sizeof(int)*size/2);
          goto error_end;
        }
        ...
        for ( j=0 ; j<pdx->tone[num].size ; j++ ) {
          if (address+j*2+1<buflen) {
            ...
            conout("PDX.%d: RAW16 Address overflow. %d>=%d.\n",num,address+j*2+1,buflen);
            goto error_end;
          }
        ...
        pdx->tone[num].data = (int *)malloc(sizeof(int)*size*2);
        if (!pdx->tone[num].data) {
          conout("PDX.%d: ADPCM Memory overflow. %dbytes.\n",num,sizeof(int)*size*2);
          goto error_end;
        }
        pdx->tone[num].size = size*2;
...
static void adpcm2pcm( int *dst, const u8 *src, long size ) {
  int i,j;
  adpcm_val adpcm;
