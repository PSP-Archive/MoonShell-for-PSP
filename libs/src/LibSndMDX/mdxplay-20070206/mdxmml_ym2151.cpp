...
static int fade_out_wait;
static int master_volume;
static double TimerB_Last;

int
mdx_parse_mml_ym2151_Start( MDX_DATA *orig_mdx, PDX_DATA *orig_pdx , u32 SampleRate, u32 SamplesPerFrame)
{
  __GETSELF;
  ...
  signal_init();
  if ( self->mdx->is_output_to_stdout == FLAG_FALSE ) {
    priority_init();
  }

  self->pcm8_disable=FLAG_TRUE;
  if ( pcm8_open(self->mdx,SampleRate,SamplesPerFrame)==0 ) {
...
bool
mdx_parse_mml_ym2151_Loop(u32 *pPCMBuf32,double FrameTime)
{
  __GETSELF;
  
  TimerB_Last+=FrameTime;
  
  while(0<=TimerB_Last) {
    double clockmsec=1024*(256 - self->mdx->tempo)/4000;
    TimerB_Last-=clockmsec/1000;
    
    if(all_track_finished==FLAG_TRUE) return(false);
    
    if ( self->fade_out > 0 ) {
      if ( fade_out_wait==0 ) { fade_out_wait = self->fade_out; }
      fade_out_wait--;
      if ( fade_out_wait==0 ) { master_volume--; }
      if ( master_volume==0 ) { return(false); }
    }
    ...
    int infinite_loops = 32767; /* large enough */
    for ( int i=0 ; i<self->mdx->tracks ; i++ ) {

      if ( self->mdx->track[i].waiting_sync == FLAG_TRUE ){ continue; }

      long count = self->mdx->track[i].counter;
  ...
  if(pPCMBuf32!=NULL) do_pcm8(pPCMBuf32);
  
  return(true);
}

void
mdx_parse_mml_ym2151_End(void)
{
  __GETSELF;
...
static RETSIGTYPE
signal_vector( int num )
{
  ym2151_all_note_off();

  /*pcm8_close();*/
  conout("Signal caught : %d\n", num);
  exit(1);
}

static void
signal_play_stop( int num )
{
  int i;
  __GETSELF;
  ...
  if ( self->is_sigint_caught == FLAG_TRUE ) return;
  for ( i=0 ; i<self->mdx->tracks ; i++ ) {
    self->mdx->track[i].counter = -1;
  }
  self->mdx->is_normal_exit = FLAG_FALSE;
  self->is_sigint_caught = FLAG_TRUE;
...
signal_init( void )
{
  ...
  self->is_sigint_caught    = FLAG_FALSE;
  self->mdx->is_normal_exit = FLAG_TRUE;
  return;
}
