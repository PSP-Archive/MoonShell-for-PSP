...
void _verify(OggVorbis_File *ov,
  ...
  if(val!=-1 && ov_raw_tell(ov)<val){
    fconout(stderr,"raw position out of tolerance: requested %ld, got %ld\n",
           (long)val,(long)ov_raw_tell(ov));
    exit(1);
  }
  if(pcmval!=-1 && ov_pcm_tell(ov)>pcmval){
    fconout(stderr,"pcm position out of tolerance: requested %ld, got %ld\n",
           (long)pcmval,(long)ov_pcm_tell(ov));
    exit(1);
  }
  if(timeval!=-1 && ov_time_tell(ov)>timeval){
    fconout(stderr,"time position out of tolerance: requested %ld, got %ld\n",
            (long)timeval,(long)ov_time_tell(ov));
    exit(1);
  }
  pos=ov_pcm_tell(ov);
  if(pos<0 || pos>pcmlength){
    fconout(stderr,"pcm position out of bounds: got %ld\n",(long)pos);
    exit(1);
  }
  ...
  if(bigassbuffer){
	...
	fconout(stderr,"data position after seek doesn't match pcm position\n");
	
	{
	  FILE *f=fopen("a.m","w");
	  for(j=0;j<bread;j++)fconout(f,"%d\n",(int)buffer[j]);
	  fclose(f);
	  f=fopen("b.m","w");
...
int main(){
  ...
  if(ov_open(stdin, &ov, NULL, 0) < 0) {
    fconout(stderr,"Could not open input as an OggVorbis file.\n\n");
    exit(1);
  }
  ...
  if(ov_seekable(&ov)){
    ...
    for(i=0;i<ov.links;i++){
      ...
      if(vi->channels!=2){
        fconout(stderr,"Sorry; right now seeking_test can only use Vorbis files\n"
	...
	fconout(stderr,"\rloading.... [%ld left]              ",
		(long)(pcmlength*4-i));
      }
    }else{
      fconout(stderr,"\rfile too large to load into memory for read tests;\n\tonly verifying seek positioning...\n");
    }
    
    {
      ogg_int64_t length=ov.end;
      fconout(stderr,"\rtesting raw seeking to random places in %ld bytes....\n",
             (long)length);
    
      for(i=0;i<1000;i++){
        ogg_int64_t val=rand()*length/RAND_MAX;
        fconout(stderr,"\r\t%d [raw position %ld]...     ",i,(long)val);
        ret=ov_raw_seek(&ov,val);
        if(ret<0){
          fconout(stderr,"seek failed: %d\n",ret);
          exit(1);
        }
    ...
    fconout(stderr,"\r");
    {
      fconout(stderr,"testing pcm page seeking to random places in %ld samples....\n",
             (long)pcmlength);
    
      for(i=0;i<1000;i++){
        ogg_int64_t val=(double)rand()*pcmlength/RAND_MAX;
        fconout(stderr,"\r\t%d [pcm position %ld]...     ",i,(long)val);
    ...
    fconout(stderr,"\r");
    {
      fconout(stderr,"testing pcm exact seeking to random places in %ld samples....\n",
             (long)pcmlength);
      ...
      for(i=0;i<1000;i++){
        ...
        if(ov_pcm_tell(&ov)!=val){
          fconout(stderr,"Declared position didn't perfectly match request: %ld != %ld\n",
    ...
    fconout(stderr,"\r");
    {
      fconout(stderr,"testing time page seeking to random places in %ld milliseconds....\n",
              (long)timelength);
    
      for(i=0;i<1000;i++){
        ogg_int64_t val=(double)rand()*timelength/RAND_MAX;
        fconout(stderr,"\r\t%d [time position %ld]...     ",i,(long)val);
    ...
    fconout(stderr,"\r");
    {
      fconout(stderr,"testing time exact seeking to random places in %ld milliseconds....\n",
              (long)timelength);
    ...
    fconout(stderr,"Standard input was not seekable.\n");
  }
