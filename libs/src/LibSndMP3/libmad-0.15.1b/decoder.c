...
# endif
      ...
      if (decoder->header_func) {
        if (mad_header_decode(&frame->header, stream) == -1) {
          ...
          switch (error_func(error_data, stream, frame)) {
            case MAD_FLOW_STOP: goto done;
            case MAD_FLOW_BREAK: goto fail;
            case MAD_FLOW_IGNORE: case MAD_FLOW_CONTINUE: default: continue;
          }
        }
      ...
      if (mad_frame_decode(frame, stream) == -1) {
        ...
        switch (error_func(error_data, stream, frame)) {
          ...
          case MAD_FLOW_CONTINUE: default: continue;
        }
        }else{
        bad_last_frame = 0;
      }
      ...
      if (decoder->output_func) {
        switch (decoder->output_func(decoder->cb_data, &frame->header, &synth->pcm)) {
          case MAD_FLOW_STOP: goto done;
          case MAD_FLOW_BREAK: goto fail;
          case MAD_FLOW_IGNORE: case MAD_FLOW_CONTINUE: break;
        }
      }
    }
  }
...
int mad_decoder_oneframe_start(struct mad_decoder *decoder)
{
  decoder->sync = malloc(sizeof(*decoder->sync));
  if (decoder->sync == 0) return(0);
  
  if (decoder->input_func == 0) return(0);

  if (decoder->error_func == 0) return(0);
...
int mad_decoder_oneframe_sync(struct mad_decoder *decoder,u32 *pBufLR)
{
  enum mad_flow (*error_func)(void *, struct mad_stream *, struct mad_frame *) = decoder->error_func;
  void *error_data = decoder->cb_data;
  int bad_last_frame = 0;
  ...
  while(1) {
    ...
    while (1) {
      ...
      if(pBufLR==NULL){
        unsigned int ns=MAD_NSBSAMPLES(&frame->header);
        u32 pcmlength=32*ns;
        if (frame->options & MAD_OPTION_HALFSAMPLERATE) pcmlength/=2;
        return(pcmlength);
      }
      ...
      if(synth->pcm.length!=0){
        if (decoder->output_func) {
          switch (decoder->output_func(pBufLR, &frame->header, &synth->pcm)) {
            case MAD_FLOW_STOP: goto done;
            case MAD_FLOW_BREAK: goto fail;
            case MAD_FLOW_IGNORE: break;
            case MAD_FLOW_CONTINUE: return(synth->pcm.length);
          }
        }
      }
    }
    
    if(stream->error!=MAD_ERROR_BUFLEN) goto fail;
  }
...
void mad_decoder_oneframe_end(struct mad_decoder *decoder)
{
  struct mad_stream *stream = &decoder->sync->stream;
  struct mad_frame *frame = &decoder->sync->frame;
  struct mad_synth *synth = &decoder->sync->synth;
