...
static void BGParticle_Init(void)
{
  ...
  Texture_CreateFromFile(true,EVMM_Process,&FL_Particle32Tex,ETF_RGBA8888,Resources_FileListPath "/FL_Particle32.png");
...
static void BGParticle_Draw(void)
{
  ...
  for(u32 idx=0;idx<BGParticlesCount;idx++){
    ...
    if(pbp->Life!=0){
      ...
      if(a<0x00) a=0x00;
      if(0xff<a) a=0xff;
      Texture_GU_DrawResize(&FL_Particle32Tex,pbp->x-s,pbp->y-s,s,s,((a/4)<<24)|0x00ffe0e0);
    }
  }
...
static void BGImg_Reload(void)
{
  ...
  if(FileExists(pfn)==false){
    pBGImg=ImageLoadFromFile(Resources_FileListPath "/FL_BG.png",ScreenWidth,ScreenHeight);
    }else{
    pBGImg=(u32*)safemalloc(ScreenWidth*ScreenHeight*4);
    FILE *pf=fopen(pfn,"r");
    fread(pBGImg,ScreenWidth*ScreenHeight*4,1,pf);
    fclose(pf);
  }
}
